...
class NeuralNetworkTensorflow:
    ...
    def create_DNN(self):
        with tf.name_scope('DNN'):
            ...
            # Output layer
            self.W_out = self.weight_variable([self.n_neurons_layer1, self.n_categories], name='out', dtype=tf.float32)
            b_out = self.bias_variable([self.n_categories], name='out', dtype=tf.float32)
            self.z_out = tf.matmul(a_fc1, self.W_out) + b_out
    ...
    def create_loss(self):
        with tf.name_scope('loss'):
            ...
            regularizer_loss = self.lmbd*(regularizer_loss_fc1 + regularizer_loss_out)
